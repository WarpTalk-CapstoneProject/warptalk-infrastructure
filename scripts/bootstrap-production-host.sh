...
if [ "$ROLE" = "app" ]; then
  ...
  for port in 5432 6432 6379 5672 15672 15692 9000 9001 6333 6334 4317 4318 5341 9090 3001; do
    run ufw allow from "$APP_PRIVATE_IP" to any port "$port" proto tcp
  done
fi

run ufw --force enable
run install -d -m 0750 /opt/warptalk /etc/warptalk /var/backups/warptalk
